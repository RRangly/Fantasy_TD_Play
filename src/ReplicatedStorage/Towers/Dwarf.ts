...
class Dwarf extends Tower {
    ...
    actionUp(deltaTime: number, mobs: Array<Mob>): void | AttackInstance {
        const stat = DwarfInfo.stats[this.level]
        if (mobs) {
...
//Factory function
export function newDwarf(position: Vector3, model: Model) {
    return new Dwarf(position, model)
}