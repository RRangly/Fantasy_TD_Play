...
class Cleric extends Tower {
    ...
    actionUp(deltaTime: number, mobs: Array<Mob>): void | AttackInstance {
        const stat = ClericInfo.stats[this.level]
        if (mobs) {
...
//Factory function
export function newCleric(position: Vector3, model: Model) {
    return new Cleric(position, model)
}