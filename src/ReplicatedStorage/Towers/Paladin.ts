...
class Paladin extends Tower {
    ...
    actionUp(deltaTime: number, mobs: Array<Mob>): void | AttackInstance {
        const stat = PaladinInfo.stats[this.level]
        if (mobs) {
...
//Factory function
export function newPaladin(position: Vector3, model: Model) {
    return new Paladin(position, model)
}